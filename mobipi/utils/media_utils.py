...
def to_fisheye(image):
    # Define fisheye parameters (adjust for effect)
    dim = image.shape[:2]
    K = np.array([[dim[1]//2, 0, dim[1]//2],  # fx, 0, cx
                  [0, dim[0]//2, dim[0]//2],  # 0, fy, cy
                  [0, 0, 1]], dtype=np.float32)  # Perspective

    D = np.array([10., 10., 0., 0.])  # Distortion coefficients (tweak as needed)

    # Apply fisheye effect
    new_K = cv2.fisheye.estimateNewCameraMatrixForUndistortRectify(K, D, dim[::-1], None)
    map1, map2 = cv2.fisheye.initUndistortRectifyMap(K, D, np.eye(3, dtype=np.float32), new_K, dim[::-1], cv2.CV_16SC2)
    fisheye_img = cv2.remap(image, map1, map2, interpolation=cv2.INTER_LINEAR)

    mask = np.zeros_like(fisheye_img)
    cv2.circle(mask, (dim[1]//2, dim[0]//2), dim[1]//2, (255, 255, 255), -1)
    fisheye_img = cv2.bitwise_and(fisheye_img, mask)
